...
const BreadCrumbsPage = () => {
  ...
  const getChapterApi = async () => {
    try {
      ...
      const data = await response.json();
      setChapterNumber(data?.data?.chapterNumber);
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <ol className="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li aria-current="page">
            <div className="flex items-center">
              ...
              <Link
                href={`/auth/units/${selectedId}/${selectChapterId}/page`}
                className="ml-1 text-sm font-medium text-gray-700 hover:scale-110 transition-transform duration-300 md:ml-2"
              >
                Page - {pageNumber}
              </Link>
